...
const DropdownMoreFilters = ({
      ...
      return (
        ...
        >
          <View className="flex-row items-center flex-1">
            ...
            <Text className="text-gray-800 mr-6">
              {item.label.split("-")[0]} {item.label.split("-")[1]}
            </Text>
          </View>
          <View className="mx-2">
            <Text className="text-xs text-gray-600 bg-[#e5e7eb] rounded-[4px] px-2">
              {item.count}
            </Text>
          </View>
        </TouchableOpacity>
  ...
  return (
    <View style={{ position: "relative" }}>
      ...
      >
        <Text className={isOpen ? "text-white" : "text-gray-700"}>{title}</Text>
        <Ionicons

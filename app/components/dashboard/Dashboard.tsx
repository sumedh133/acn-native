...
const RequirementCard = React.memo(
  ({
    requirement,
    onStatusChange,
    index,
    ...
    const currentStatus =
      statusOptions.find((option) => option.label === requirement.status) ||
      statusOptions[0];
...
export default function Dashboard({
  ...
  const requirementMonthOptions = useRef(
    generateRequirementMonths(myRequirements),
  );
  ...
  useEffect(() => {
    ...
    if (myRequirements) {
      ...
      requirementMonthOptions.current =
        generateRequirementMonths(myRequirements);
    }
    if (myEnquiries) {
  ...
  const [myInverntoriesIconColor, setMyInverntoriesIconColor] =
    useState("white");
  const [myRequirementsIconColor, setMyRequirementsIconColor] =
    useState("#153E3B");
  ...
  const tabData = [
    {
      ...
      icon: (
        <MyInverntoriesIcon
          height={36}
          width={36}
          iconColor={myInverntoriesIconColor}
        />
      ),
